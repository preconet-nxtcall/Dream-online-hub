...
class _RechargeNowWidgetState extends State<RechargeNowWidget> {
  ...
  Future<void> _submitForm() async {
    ...
    if (parsedAmount == null || parsedAmount <= 0) {
      _showError('Please enter a valid recharge amount.');
      return;
    }
    final txnId = _txnIdController.text.trim();
    if (txnId.isEmpty) {
      _showError('Please enter the Transaction ID or UTR.');
      return;
    }
    ...
    try {
      ...
      final data = response.data;
      if (data is Map<String, dynamic> && data['success'] == true) {
        final successMsg = data['message']?.toString() ?? 'Recharge Submitted Successfully!';
        ...
        await LocalStorageRepositoryImpl().saveSubmittedRecharge(newRecord);
        widget.onRechargeSubmitted?.call(newRecord);
      ...
      if (!mounted) return;
      setState(() => _isSubmitting = false);
      String errMsg = 'Failed to submit recharge request. Please check your network connection.';
