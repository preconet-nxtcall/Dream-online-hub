document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    if (cached) {
      ...
      if (parsed && parsed.user) {
        ...
        if (parsed.user.avatar) {
          displayAvatar = formatAvatarUrl(parsed.user.avatar) || 'images/avatar.avif';
        } else {
    ...
    <a id="nav-home" href="/chat/view/home.html" class="flex items-center gap-2 text-text-muted hover:text-primary transition-colors">
    ...
    <a id="nav-recharge" href="/chat/view/recharge.html" class="flex items-center gap-2 text-text-muted hover:text-primary transition-colors">
    ...
    <a id="nav-records" href="/chat/view/records.html" class="flex items-center gap-2 text-text-muted hover:text-primary transition-colors">
    ...
    <div id="mobile-drawer" class="fixed inset-y-0 right-0 z-50 w-64 bg-[#1e1f26]/95 backdrop-blur-xl border-l border-white/10 p-6 flex flex-col gap-6 shadow-2xl transition-transform duration-300 transform translate-x-full">
      ...
      <nav class="flex flex-col gap-4">
        ...
        <a id="mobile-nav-home" href="/chat/view/home.html" class="flex items-center gap-3 p-3 rounded-lg text-text-muted hover:text-primary hover:bg-white/5 transition-colors">
          <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 0;">home</span>
          <span class="font-label-caps text-label-caps font-semibold">HOME</span>
        </a>
        <a id="mobile-nav-recharge" href="/chat/view/recharge.html" class="flex items-center gap-3 p-3 rounded-lg text-text-muted hover:text-primary hover:bg-white/5 transition-colors">
          <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 0;">account_balance_wallet</span>
          <span class="font-label-caps text-label-caps font-semibold">RECHARGE</span>
        </a>
        <a id="mobile-nav-records" href="/chat/view/records.html" class="flex items-center gap-3 p-3 rounded-lg text-text-muted hover:text-primary hover:bg-white/5 transition-colors">
          <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 0;">history</span>
  ...
  <div id="profileModal" class="profile-modal" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.65); backdrop-filter: blur(10px); z-index: 10000; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.25s ease; padding: 1rem;">
    ...
    const showProfileModal = async () => {
      ...
      try {
        ...
        document.getElementById('modalProfileImg').src = formatAvatarUrl(user.avatar) || 'images/avatar.avif';
        ...
        // Fetch live profile from MySQL via /api/v1/auth/me
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (data && data.user) {
                ...
                if (fresh.mob) document.getElementById('modalProfilePhone').textContent = fresh.mob;
                if (fresh.avatar || fresh.img) {
                  document.getElementById('modalProfileImg').src = formatAvatarUrl(fresh.avatar || fresh.img) || 'images/avatar.avif';
                }
                if (fresh.name) document.getElementById('modalProfileName').textContent = fresh.name;

                // Cache fresh user in localStorage
                user.mob = fresh.mob || user.mob;
                user.avatar = fresh.avatar || fresh.img || user.avatar;
                user.name = fresh.name || user.name;
                parsed.user = user;
                localStorage.setItem('chat_identity', JSON.stringify(parsed));

                // Also update trigger button avatars if present
                const dtImg = document.getElementById('profile-trigger-desktop')?.querySelector('img');
                if (dtImg) dtImg.src = formatAvatarUrl(user.avatar) || 'images/avatar.avif';
                const mbImg = document.getElementById('profile-trigger-mobile')?.querySelector('img');
                if (mbImg) mbImg.src = formatAvatarUrl(user.avatar) || 'images/avatar.avif';
              }
            }
          } catch (_) {}
        }
    ...
    const handleBackToAdmin = (e) => {
        ...
        if (agentIdentity) {
            try {
                ...
                window.location.href = '/chat/admin.html';
            } catch (err) {
