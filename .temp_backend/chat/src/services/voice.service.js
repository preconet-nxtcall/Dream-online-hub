...
export async function generatePresignedUploadUrl({ conversationId, senderId, mimeType = 'audio/webm' }) {
  const extension = mimeType.split('/')[1] || 'webm';
  // Include conversationId and senderId in file storage path for clean auditing
  const fileKey = `voice-notes/${conversationId}/${senderId || 'anonymous'}/${uuidv4()}.${extension}`;
  ...
  if (isMock) {
    console.log('[VoiceService] Using local mock upload configuration');
    return {
      provider: 'Local Mock',
      uploadUrl: `/api/v1/voice/upload-mock?key=${fileKey}`,
      fileKey,
  ...
  try {
    ...
    const metadata = {
      'x-amz-meta-conversationid': String(conversationId || ''),
      'x-amz-meta-senderid': String(senderId || '')
    };
    ...
    console.warn('[VoiceService] Fallback to local mock upload URL:', err.message);
    return {
      provider: 'Local Mock',
      uploadUrl: `/api/v1/voice/upload-mock?key=${fileKey}`,
      fileKey,
...
export async function generatePresignedDownloadUrl({ fileKey }) {
  ...
  try {
    ...
    console.log(`[VoiceService] Serving file locally from mock: ${fileKey}`);
    return { url: `/uploads/${fileKey}` };
  } catch (e) {
  ...
  try {
    ...
    console.error('[VoiceService] Failed to generate presigned download URL:', err.message);
    throw err;
  }
}
