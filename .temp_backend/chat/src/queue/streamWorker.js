...
export async function processStreamMessage(streamMessageId, rawPayload) {
  try {
    ...
    const p1 = participant1 || sorted[0];
    const p2 = participant2 || sorted[1];
    const incField = recipientId === p1 ? 'unread1' : 'unread2';

    let convAgentId = recipientId;
    let convEmailId = senderId;
    if (senderType === 'agent' || senderType === 'admin') {
      convAgentId = senderId;
      convEmailId = recipientId;
    }

    const lastMsgText = text || (type === 'voice' ? '🎤 Voice note' : (type === 'image' ? '📷 Image' : (type === 'recharge' ? '💸 Recharge Request' : (type === 'withdraw' ? '🏦 Withdrawal Request' : 'New message'))));

    // 1. Upsert Conversation & update unread count + lastMessageAt FIRST
    try {
      ...
      console.log(`[StreamWorker] Conversation ${conversationId} persisted successfully.`);
    } catch (convErr) {
      console.error(`[StreamWorker] Error saving conversation ${conversationId}:`, convErr.message);
    }

    // 2. Persist message to MongoDB (safely upsert using findByIdAndUpdate)
    let createdMessage = null;
    try {
      createdMessage = await Message.findByIdAndUpdate(
        _id,
        {
          $setOnInsert: {
            _id,
      ...
      console.log(`[StreamWorker] Message ${_id} persisted successfully.`);
    } catch (msgErr) {
      console.error(`[StreamWorker] Error saving message ${_id}:`, msgErr.message);
    }

    const messageObj = createdMessage ? createdMessage.toObject() : {
      _id,
      conversationId,
      senderId,
      senderType: senderType || 'agent',
    ...
    if (messageObj.type === 'withdraw' && messageObj.withdraw && messageObj.withdraw.proofImage) {
      ...
      if (isMock) {
      ...
      } else if (messageObj.withdraw.proofImage.startsWith('images/')) {
        try {
        ...
        messageObj.withdraw.proofImageCdnUrl = `/uploads/photos/${messageObj.withdraw.proofImage}`;
      }
    }
