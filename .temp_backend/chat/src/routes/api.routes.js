...
function requireAuth(req, res, next) {
  ...
  if (!token) {
    return res.status(401).json({ error: 'Authorization header or token cookie required' });
  }
...
// Withdraw routes
router.get('/withdraw/payment-account', rechargeController.getPaymentAccount);
router.post('/withdraw/payment-account', rechargeController.getPaymentAccount);
