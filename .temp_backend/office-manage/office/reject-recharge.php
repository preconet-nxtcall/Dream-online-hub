...
if(ISSET($_POST['modal_close'])){
    $showModal = "";
    header('Location: reject-recharge');
}
$msg='';
?>
...
<div class="content mb-4">
    ...
    <div class="card cntnt-start border-rounded shadow mx-4" >
        <div class="card-header text-center gradient-15" >
            <h5 class="card-title text-white my-1"><strong>Rejected Recharge</strong> List</h5>
        </div>
                ...
                <table class="display table table-hover text-center" id="example" style="min-width: auto;">
                    ...
                    <?php
                        ...
                        if($_SESSION['u_type'] == "ADMIN"){
                            $qrydisplay = mysqli_query($conn, "SELECT * FROM `recharge` WHERE agency_read_status = 'READ' AND stage_status = 'AGENCY-REJECT' ORDER BY ABS(id) DESC") or die(mysqli_error());
                        }elseif($_SESSION['u_type'] == "AGENCYS-EMPLOYEE" || $_SESSION['u_type'] == "AGENCY"){
                            $qrydisplay = mysqli_query($conn, "SELECT * FROM `recharge` WHERE emp_id = '$emp_id' AND agency_read_status = 'READ' AND stage_status = 'AGENCY-REJECT' ORDER BY ABS(id) DESC") or die(mysqli_error());
                        }
                    ...
                    ?>
                        <tr id="<?php echo $result['id']; ?>">
                            ...
                            <td>Amount : <?php echo $result['amount']; ?> 
                                <br> Transaction ID : <?php echo $result['transection_id']; ?>
                                <?php 
                                    if($_SESSION['u_type'] == "ADMIN"){ 
                                        $qryagnc = mysqli_query($conn, "SELECT * FROM `users` WHERE id = '$result[emp_id]' ") or die(mysqli_error());
                                        $reagnc = mysqli_fetch_array($qryagnc);
                                ?>
                                <br> Agency Name : <?php echo $reagnc['name']; ?>
                                <?php } ?>
                            </td>
                            <td>
                                            ...
                                            <div class="modal-header shadow bg-funky-moon2 mx-auto">
                                                <div class="row mb-1">
                                                    ...
                                                    <div class="col-4 text-right">
                                                        <form action="reject-recharge" method="POST">
