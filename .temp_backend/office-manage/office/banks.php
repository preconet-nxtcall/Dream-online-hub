...
if(ISSET($_POST['add_bank'])){
    ...
    $slag = rtrim($slag, "-");
    $order_no = $_SESSION['u_id'];
    $detail = addslashes($_POST["detail"]);
    $type = "BANK";
    $show_status = "ACTIVE";
    $msg = '';
    
    $res1=mysqli_query($conn,"select * from features where slag = '$slag' and order_no = '$order_no' and type='$type'");
	$check=mysqli_num_rows($res1);
	if($check>0){
		$msg="1";
        $_SESSION['swl_type'] = "error";
        $_SESSION['head'] = "Error !";
        $_SESSION['text'] = "Bank already exist!";
    ...
    if($msg == ''){
        $qry = "INSERT INTO `features`(`name`, `slag`, `order_no`, `detail`, `show_status`, `type`, `date_ts`) VALUES ('".$name."','".$slag."','".$order_no."','".$detail."','".$show_status."','".$type."','".$date_ts."')";
        $query = mysqli_query($conn,$qry);
        if($query){
            $_SESSION['swl_type'] = "success";
            $_SESSION['head'] = "Successfull !";
            $_SESSION['text'] = "Bank Add Successfully!";
            if(ISSET($_SESSION['swl_type'])){
                header("Refresh:0;");
                exit;
            }
...
if(ISSET($_POST['edit_bank'])){
    ...
    $res1=mysqli_query($conn,"select * from features where id <> '$id' and slag = '$slag' and order_no = '$order_no' and type='$type'");
	$check=mysqli_num_rows($res1);
	if($check>0){
		$msg="1";
        $_SESSION['swl_type'] = "error";
    ...
    $qry = "UPDATE `features` set `name` = '".$name."', `slag` = '".$slag."', `order_no` = '".$order_no."', `detail` = '".$detail."' WHERE `id` = '".$id."'" or die(mysqli_error());
    $query = mysqli_query($conn,$qry);
    ...
    if($query){
        ...
        $_SESSION['swl_type'] = "success";
        $_SESSION['head'] = "Successfull !";
        $_SESSION['text'] = "Bank Update Successfully!";
        if(ISSET($_SESSION['swl_type'])){
            header("Refresh:0;");
            exit;
        }
...
<div class="content mb-4">
    ...
    <div class="brdcmp px-4 pt-4">
        <div class="d-flex justify-content-between align-items-center pb-4 mb-3">
            ...
            <button type="button" data-bs-toggle="modal" data-bs-target="#insert_partnr" class="btn btn_warning btn-sm">Add Bank<i class="bi bi-arrow-right ms-2"></i></button>
        </div>
    </div>

    <div class="card cntnt-start border-rounded shadow mx-4" >
        <div class="card-header text-center gradient-15" >
            <h5 class="card-title text-white my-1"><strong>Banks </strong> List</h5>
        </div>
        <div class="card-body pb-0">
            ...
            <div class="modal fade mt-4 pt-4" id="insert_partnr" tabindex="3">
                <div class="modal-dialog modal-dialog-scrollable modal-lg">
                    <div class="modal-content" style="overflow: visible!importent;">
                        <div class="modal-header shadow bg-funky-moon2 mx-auto">
                            <div class="row mb-1">
                                <div class="col-8 text-left">
                                    <h5 class="modal-title"><b>Add Bank</b></h5>
                                </div>
                        ...
                        <form class="row" action="banks" method="post" enctype="multipart/form-data" id="form">
                            <div class="card-body card-block">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-8">
                                        <div class="input-group mb-3">
                                            ...
                                            <input type="text" class="form-control" name="name" placeholder="Enter Bank Account No" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-8">
                                        <div class="input-group mb-3">
                                            <span class="input-group-text" id="basic-addon1">✎</span>
                                            <textarea class="form-control" name="detail" placeholder="Enter Account Details"></textarea>
                                        </div>
                                    </div>
                                </div>
                                <hr class="ml-100">
                                <div class="row">
                                    <div class="d-flex gap-3 mt-3">
                                        <button name="add_bank" type="submit" class="mx-auto btn btn-lg one-click btn_primary wave-effect">
            ...
            <div class="table-responsive px-2">
                <table class="display table table-hover text-center" id="example" style="min-width: auto;">
                    ...
                    <?php
                        $i = 0;
                        $qrydisplay = mysqli_query($conn, "SELECT * FROM `features` WHERE order_no = '".$_SESSION['u_id']."' AND type = 'BANK' ORDER BY ABS(id) DESC") or die(mysqli_error());
                        ...
                        <tr id="<?php echo $result['id'] ?>" >
                            ...
                            <td> 
                                    ...
                                    <div class="modal-dialog modal-dialog-scrollable modal-lg">
                                        <div class="modal-content" style="overflow: visible!importent;">
                                            <div class="modal-header shadow bg-funky-moon2 mx-auto">
                                                <div class="row mb-1">
                                                    <div class="col-8 text-left">
                                                        <h5 class="modal-title text-white ml-1"><b>Update Bank</b></h5>
                                                    </div>
                                                    <div class="col-4 text-right">
                                                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                            ...
                                            <div class="modal-body">
                                                <div class="container-fluid text-dark">
                                                    <form class="row px-2" action="banks" method="post" enctype="multipart/form-data">
                                                        ...
                                                        <div class="card-body card-block">
                                                            ...
                                                            <hr class="md-100">
                                                            <div class="row mb-6">
                                                                <label class="col-sm-4 col-form-label">Bank Account Details</label>
                                                                <div class="col-sm-8">
                                                                    <div class="input-group mb-3">
                                                                        <span class="input-group-text" id="basic-addon1">✎</span>
                                                                        <textarea class="form-control" name="detail" required><?php echo $result['detail']; ?></textarea>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <hr class="md-100">
                                                            <div class="row">
                                                                <div class="d-flex gap-3 mt-3">
                                                                    <button name="edit_bank" type="submit" class="mx-auto btn btn-lg one-click btn_primary wave-effect">
                                ...
                                <?php if($_SESSION['u_type'] == "ADMIN") { ?>
                                    <a class="text-white pr-2 <?php if($result['admin_payment_receive'] != "ADMIN-RECEIVABLE"){ ?> update_btn_money_receivable_ajax <?php } ?> " href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-title="Receive Money"><button class="btn py-1 px-2 <?php if(isset($result['admin_payment_receive']) && $result['admin_payment_receive'] == "ADMIN-RECEIVABLE"){ ?>btn_info<?php }else{ ?>btn_secondary<?php } ?>" type="button" style="vertical-align:middle"><span><i class="bi bi-wallet2"></i> </span></button></a>
                                <?php } ?>
                            </td>
                        </tr>
                    <?php  }  ?>
