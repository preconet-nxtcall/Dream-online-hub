...
<div class="content mb-4">
    ...
    <div class="card cntnt-start border-rounded shadow mx-4">
        <div class="card-header text-center gradient-15">
            <h5 class="card-title text-white my-1"><strong>Pending User Bank Accounts</strong> List</h5>
        </div>
                ...
                <table class="display table table-hover text-center" id="example" style="min-width: auto;">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center" scope="col">Account Details</th>
                    ...
                    <?php
                        $i = 0;
                        $qrydisplay = mysqli_query($conn, "SELECT * FROM `user_payment_accounts` WHERE `stage_status` = 'EMPLOYEE-PENDING' ORDER BY ABS(id) DESC") or die(mysqli_error($conn));
                    ...
                    ?>
                        <tr id="row_<?php echo $result['id']; ?>">
                            ...
                            <td>
                                ...
                                Phone: <?php echo isset($resusr['mob']) ? htmlspecialchars($resusr['mob']) : 'N/A'; ?><br>
                                Email: <?php echo isset($resusr['email']) ? htmlspecialchars($resusr['email']) : 'N/A'; ?>
                            </td>
                            <td>
                                <b>Holder:</b> <?php echo htmlspecialchars($result['account_name']); ?><br>
                                <b>Acc No:</b> <?php echo htmlspecialchars($result['account_no']); ?><br>
                                <b>Bank:</b> <?php echo htmlspecialchars($result['bank_name']); ?><br>
                                <b>IFSC:</b> <?php echo htmlspecialchars($result['ifsc_code']); ?><br>
                                <b>UPI ID:</b> <?php echo htmlspecialchars($result['upi_id']); ?>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-1">
                                    ...
                                    <button type="button" class="btn btn_primary py-1 px-2" data-bs-toggle="modal" data-bs-target="#viewModal<?php echo $result['id']; ?>" title="View Details">
                                        <i class="bi bi-eye"></i>
                                    </button>

                                    <!-- Approve Form -->
                                    <form action="emp-user-bank-ac-pending" method="POST" style="display:inline;">
                                        <input type="hidden" name="bank_ac_id" value="<?php echo $result['id']; ?>">
                                        <button type="button" onclick="confirmApproveBankAc(this)" class="btn btn-success py-1 px-2" title="Approve">
                                    ...
                                    <form action="emp-user-bank-ac-pending" method="POST" style="display:inline;">
                                        <input type="hidden" name="bank_ac_id" value="<?php echo $result['id']; ?>">
                                        <button type="button" onclick="confirmDeleteBankAc(this)" class="btn btn-danger py-1 px-2" title="Delete">
