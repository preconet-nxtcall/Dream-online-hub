...
if(ISSET($_POST['add_agency'])){
    ...
    $collect_limit = addslashes($_POST["collect_limit"]);
    $show_status = "ACTIVE";
    $profit_loss_status = "INACTIVE";
    $msg = '';
	...
	if($check>0){
        ...
        $_SESSION['head'] = "Error !";
        $_SESSION['text'] = "Agency already exist!";
    ...
    if($_FILES['image']['name']!=''){
        if (($_FILES['image']['size'] > 1048576) || ( $_FILES['image']['type']!='image/png' && $_FILES['image']['type']!='image/jpg' && $_FILES['image']['type']!='image/jpeg' && $_FILES['image']['type']!='image/webp' && $_FILES['image']['type']!='image/avif')){
            ...
            $img_new1=$date_ts.'_Agency.'.pathinfo($img1, PATHINFO_EXTENSION);
    ...
    if($msg == ''){
        $qry = "INSERT INTO `users`(`name`, `mob`, `email`, `password`, `img`, `profit_loss_status`, `collect_limit`, `type`, `show_status`, `date`) VALUES ('".$name."','".$mob."','".$email."','".$password."','".$img_new1."','".$profit_loss_status."','".$collect_limit."','".$type."','".$show_status."','".$date."')";
        $query = mysqli_query($conn,$qry);
        if($query){
            $unqid = mysqli_insert_id($conn);
            $agency_id = "AGENCY-".$unqid;
            $qry1 = "UPDATE `users` SET `agency_unq_id` = '".$agency_id."' WHERE `id` = '".$unqid."'";
            $query1 = mysqli_query($conn,$qry1);

            $chk_tot = mysqli_query($conn, "SELECT COUNT(*) as total FROM `users` WHERE `type` = 'AGENCY'");
            if($chk_tot && $row_tot = mysqli_fetch_assoc($chk_tot)){
                if((int)$row_tot['total'] === 1){
                    mysqli_query($conn, "UPDATE `users` SET `agency_id` = 'FEATURED' WHERE `id` = '".$unqid."'");
                } else {
                    mysqli_query($conn, "UPDATE `users` SET `agency_id` = 'NON-FEATURED' WHERE `id` = '".$unqid."'");
                }
            }

            $qry2 = "INSERT INTO `agency_cash_book`(`agency_id`, `recharge_limit_live`, `rs_inhand_expected`) VALUES ('".$unqid."', '".$collect_limit."', '0')";
            $query2 = mysqli_query($conn,$qry2);
            
            $_SESSION['swl_type'] = "success";
            $_SESSION['head'] = "Successfull !";
            $_SESSION['text'] = "Agency Add Successfully!";
            if(ISSET($_SESSION['swl_type'])){
                header("Refresh:0;");
                exit;
            }
...
if(ISSET($_POST['edit_agency'])){
    ...
    $type = "AGENCY";
    $old_collect_limit = $_POST["old_collect_limit"];
    $collect_limit = addslashes($_POST["collect_limit"]);
    ...
    $qry = "UPDATE `users` set `name` = '".$name."', `mob` = '".$mob."', `email` = '".$email."', `password` = '".$password."', `collect_limit` = '".$collect_limit."', `img` = '".$img_new1."' WHERE `id` = '".$id."'" or die(mysqli_error($conn));
    $query = mysqli_query($conn,$qry);
    ...
    if($query){
        ...
        if(!empty($collect_limit)){
            if($old_collect_limit != $collect_limit){ 
                if($old_collect_limit > $collect_limit){
                    $new_recharge_limit_live = $old_collect_limit - $collect_limit;
                    $j = 0;
                } else {
                    $new_recharge_limit_live = $collect_limit - $old_collect_limit;
                    $j = 1;
                }
                $seltagncbk = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM `agency_cash_book` WHERE `agency_id` = '" . $id . "'"));
                if($j == 0){
                    if($seltagncbk['recharge_limit_live'] < $new_recharge_limit_live){
                        $new_recharge_limit_live1 = 0;
                    } else {
                        $new_recharge_limit_live1 = $seltagncbk['recharge_limit_live'] - $new_recharge_limit_live;
                    }
                } else {
                    $new_recharge_limit_live1 = $seltagncbk['recharge_limit_live'] + $new_recharge_limit_live;
                }
                $qry2 = "UPDATE `agency_cash_book` SET `recharge_limit_live` = '" . $new_recharge_limit_live1 . "' WHERE `agency_id` = '" . $id . "'";
                $query2 = mysqli_query($conn,$qry2);
            }
        }
        ...
        $_SESSION['swl_type'] = "success";
        $_SESSION['head'] = "Successfull !";
        $_SESSION['text'] = "Agency Update Successfully!";
        if(ISSET($_SESSION['swl_type'])){
            header("Refresh:0;");
            exit;
        }
...
<div class="content mb-4">
    ...
    <div class="brdcmp px-4 pt-4">
        <div class="d-flex justify-content-between align-items-center pb-4 mb-3">
            ...
            <button type="button" data-bs-toggle="modal" data-bs-target="#insert_partnr" class="btn btn_warning btn-sm">Add Agency <i class="bi bi-arrow-right ms-2"></i></button>
        </div>
    </div>

    <div class="card cntnt-start border-rounded shadow mx-4" >
        <div class="card-header text-center gradient-15" >
            <h5 class="card-title text-white my-1"><strong>Agencies </strong> List</h5>
        </div>
        <div class="card-body pb-0">
            ...
            <div class="modal fade mt-4 pt-4" id="insert_partnr" tabindex="3">
                <div class="modal-dialog modal-dialog-scrollable modal-lg">
                    <div class="modal-content" style="overflow: visible!importent;">
                        <div class="modal-header shadow bg-funky-moon2 mx-auto">
                            <div class="row mb-1">
                                <div class="col-8 text-left">
                                    <h5 class="modal-title"><b>Add Agency</b></h5>
                                </div>
                        ...
                        <form class="row" action="agencies" method="post" enctype="multipart/form-data" id="form">
            ...
            <div class="table-responsive px-2">
                <table class="display table table-hover text-center" id="example" style="min-width: auto;">
                    ...
                    <?php
                        $i = 0;
                        $check_agency_count = mysqli_query($conn, "SELECT COUNT(*) as total FROM `users` WHERE `type` = 'AGENCY'");
                        if ($check_agency_count && $row_ag_cnt = mysqli_fetch_assoc($check_agency_count)) {
                            if ((int)$row_ag_cnt['total'] === 1) {
                                mysqli_query($conn, "UPDATE `users` SET `agency_id` = 'FEATURED' WHERE `type` = 'AGENCY'");
                            }
                        }
                        $qrydisplay = mysqli_query($conn, "SELECT * FROM `users` WHERE type = 'AGENCY' ORDER BY ABS(id) DESC") or die(mysqli_error());
                    ...
                    ?>
                        <tr id="<?php echo $result['id'] ?>" >
                            ...
                            <td>
                                ...
                                Phone : <?php echo $result['mob']; ?> <br>
                                Profit/Loss : &nbsp;
                                <input type="hidden" class="id_value" value="<?php echo $result['id']; ?>" >
                                <a class="text-white pr-2 status_btn_user_profit_loss_ajax" href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-title="Status">
                                    <button class="btn py-1 px-2 rounded-pill <?php if($result['profit_loss_status'] == "ACTIVE"){ echo "btn_primary" ; }else{ echo "btn_secondary" ; }?>" type="button" style="vertical-align:middle">
                                    <span><?php if($result['profit_loss_status'] == "ACTIVE"){ echo "Shown" ; }else{ echo "Hide" ; }?> <i class="bi bi-<?php if($result['profit_loss_status'] == "ACTIVE"){ echo "check-circle" ; }else{ echo "x-lg" ; }?>"></i></span></button>
                                </a>
                            </td>
                            <td> 
                                <input type="hidden" class="id_value" value="<?php echo $result['id']; ?>" >
                                ...
                                <a class="text-white pr-2 status_btn_agency_featured_ajax" href="javascript:void(0)" data-bs-toggle="tooltip" data-bs-title="Featured Status"><button class="btn py-1 px-2 btn-rds
                                <?php if($result['agency_id'] == "FEATURED"){ ?> btn_warning text-white<?php }else{?> btn_secondary<?php }?>
                                " type="button" style="vertical-align:middle"><span>
                                <?php if($result['agency_id'] == "FEATURED"){ ?> <i class="bi bi-star-fill"></i><?php }else{?> <i class="bi bi-star"></i><?php }?>    
                                </span></button></a>
                                <?php if($_SESSION['u_type'] == "ADMIN") { ?>
                                <button class="btn py-1 px-2 btn_info text-white view_agency_perf_btn" type="button" data-agency-id="<?php echo $result['id']; ?>" data-agency-name="<?php echo htmlspecialchars($result['name']); ?>" style="vertical-align:middle;" data-bs-toggle="tooltip" data-bs-title="Performance Details"><i class="bi bi-graph-up-arrow"></i></button>
                                <?php } ?>
                                <a href="agency-users.php?agency_id=<?php echo $result['id']; ?>" data-bs-toggle="tooltip" data-bs-title="Agency Users" class="btn py-1 px-2 btn_dark text-white"style="vertical-align:middle"><i class="bi bi-people-fill"></i></a>
                                    ...
                                    <div class="modal-dialog modal-dialog-scrollable modal-lg">
                                        <div class="modal-content" style="overflow: visible!importent;">
                                            <div class="modal-header shadow bg-funky-moon2 mx-auto">
                                                <div class="row mb-1">
                                                    <div class="col-8 text-left">
                                                        <h5 class="modal-title text-white ml-1"><b>Update Agency</b></h5>
                                                    </div>
                                                    <div class="col-4 text-right">
                                                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                            ...
                                            <div class="modal-body">
                                                <div class="container-fluid text-dark">
                                                    <form class="row px-2" action="agencies" method="post" enctype="multipart/form-data">
                                                        <input type="hidden" value="<?php echo $result['id']; ?>" name="id"/>
                                                        <input type="hidden" value="<?php echo $result['collect_limit']; ?>" name="old_collect_limit"/>
                                                        ...
                                                        <div class="card-body card-block">
                                                            ...
                                                            <div class="row mb-3">
                                                                <label class="col-sm-4 col-form-label">Collect Limit</label>
                                                                <div class="col-sm-8">
                                                                    <div class="input-group mb-3">
                                                                        <span class="input-group-text" id="basic-addon1">✎</span>
                                                                        <input type="number" class="form-control" name="collect_limit" value="<?php echo $result['collect_limit']; ?>" required>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <hr class="md-100">
                                                            ...
                                                            <div class="row">
                                                                <div class="d-flex gap-3 mt-3">
                                                                    <button name="edit_agency" type="submit" class="mx-auto btn btn-lg one-click btn_primary wave-effect">
...
<!-- Agency Performance Details Modal -->
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-xl">
        <div class="modal-content" style="overflow: visible!importent;">
            <div class="modal-header shadow bg-funky-moon2 mx-auto">
                <div class="row mb-1">
                    <div class="col-8 text-left">
                        <h5 class="modal-title text-white ml-1"><b>Agency Performance & Current Details</b></h5>
                    </div>
                    <div class="col-4 text-right">
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
            ...
            <div class="modal-body py-2" id="agencyPerformanceModalBody">
                <div class="text-center py-5">
                    ...
                    <p class="mt-3 text-muted fw-semibold">Loading agency performance metrics...</p>
                </div>
            </div>
