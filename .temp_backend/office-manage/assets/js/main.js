...
(function ($) {
  ...
  var headerSticky = function () {
    let lastScrollTop = 0;
    let delta = 5;
    let navbarHeight = $("header").length ? $("header").outerHeight() : 0;
    let didScroll = false;
    ...
    setInterval(function () {
      if (didScroll) {
        ...
        navbarHeight = $("header").length ? $("header").outerHeight() : 0;
  ...
  var totalPriceVariant = function () {
    ...
    function updateTotalPrice() {
      var currentPrice = parseFloat($(".price-on-sale").text().replace("$", "")) || 0;
